const entities = [
    ...
    },
    {
        ...
        fields: [
            ...
            { name: 'total_price', label: 'Итоговая цена', type: 'number', step: '0.01', required: true }
        ],
        filters: [
    ...
    {
        key: 'productSales', title: 'Товары в продажах', route: '/product-sales', id: 'product_sales_id',
        columns: [
...
async function loadReferences() {
    const refRoutes = [
        ...
        ['sales', '/sales', item => `${item.datetime} / ${item.total_cost}`]
    ];
...
function renderTable(table, columns, rows, withActions) {
    ...
    rows.forEach(row => {
        ...
        columns.forEach(([key]) => {
            ...
            if (key.endsWith('_id')) td.classList.add('id-cell');
            tr.appendChild(td);
        });
        if (withActions) {
            const td = document.createElement('td');
            ...
            td.innerHTML = `
                <button class="btn btn-sm btn-outline-primary me-2" data-action="edit">Изменить</button>
                <button class="btn btn-sm btn-outline-danger" data-action="delete">Удалить</button>
            `;
            td.querySelector('[data-action="edit"]').addEventListener('click', () => openForm(row));
            td.querySelector('[data-action="delete"]').addEventListener('click', () => deleteRow(row));
            tr.appendChild(td);
        }
...
function renderInput(field, row = null, compact = false) {
    ...
    col.innerHTML = `${label}<input class="form-control" id="${id}" name="${field.name}" type="${field.type}" value="${escapeHtml(inputValue)}" ${step} ${required}>`;
    return col;
}
